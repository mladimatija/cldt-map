 ...
 * is picked up without a full reload. Silent on failure - until a dataset
 * lands the renderer treats null as "feature unavailable" and falls back to
 * default trail styling; once one is published the store keeps it through
 * failed revalidations, so the style is never downgraded after the fact.
 */
