...
/** Section index 0=A, 1=B, 2=C. Uses CSS classes (no inline styles) for CSP compatibility. */
export function sectionBoundaryIcon(label: string, sectionIndex: number): L.DivIcon {
	return L.divIcon({
		className: 'trail-section-marker',
		html: `<div class="trail-section-marker-inner trail-section-${sectionIndex}">${label}</div>`,
		iconSize: [24, 24],
		iconAnchor: [12, 12],
	});
}
