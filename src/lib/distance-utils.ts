 ...
 *
 * The two models are intentionally additive (per DR-015):
 * - Tobler adjusts the time-to-traverse-distance component based on slope.
...
export function findNearestPointIndex(
	points: { distanceFromStart: number }[],
	targetM: number,
): number {
	...
	if (lo > 0 && Math.abs(points[lo - 1].distanceFromStart - targetM) <= Math.abs(points[lo].distanceFromStart - targetM)) {
		return lo - 1;
	}
	return lo;
}
