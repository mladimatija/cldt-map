...
function isValidImportedTrack(value: unknown): value is ImportedTrack {
	...
	return (
		...
		typeof t.importedAt === 'number' && isFinite(t.importedAt) &&
...
export function computeTrackStats(
	...
	if (typeof window === 'undefined' || track.points.length === 0) {
		return { totalDistanceM: 0, totalElapsedSec: 0, totalMovingSec: 0, avgMovingPaceSecPerKm: 0, maxDeviationM: 0, coveragePercent: 0 };
	}
	...
	for (let i = 1; i < trackPoints.length; i++) {
		totalDistanceM += L.latLng(trackPoints[i - 1].lat, trackPoints[i - 1].lng).distanceTo(
			L.latLng(trackPoints[i].lat, trackPoints[i].lng),
		);
	}
	...
	if (timedPts.length >= 2) {
		totalElapsedSec =
			(timedPts[timedPts.length - 1].time!.getTime() - timedPts[0].time!.getTime()) / 1000;
	...
	const avgMovingPaceSecPerKm =
		totalDistanceM > 0 && totalMovingSec > 0
			? totalMovingSec / (totalDistanceM / 1000)
			: 0;
	...
	if (enhancedPoints.length > 0) {
		...
		for (const pt of trackPoints) {
			...
			let nearestIdx = hint;
			let nearestDist = ptLL.distanceTo(
				L.latLng(enhancedPoints[hint].lat, enhancedPoints[hint].lng),
			);

			// Forward scan from hint — break early when distances diverge significantly
			for (let j = hint + 1; j < enhancedPoints.length; j++) {
				const d = ptLL.distanceTo(L.latLng(enhancedPoints[j].lat, enhancedPoints[j].lng));
				if (d < nearestDist) {
					nearestDist = d;
					nearestIdx = j;
