...
- **Up next data-book view** - Strip in the distance HUD listing the nearest usable water source, shelter/hut, and town/settlement ahead in your direction of travel (optional food, ATM, and viewpoint rows in Settings), sorted by distance within a configurable ahead horizon (25/50/100 km); tap a row to fly to it on the map (works regardless of which marker layers are visible; toggleable in Settings)
