...
- **Pack weight & water carry** - Optional base weight + water consumption rate in Settings (stored metric, shown and entered in your active unit system - kg/L or lb/qt - and converted automatically when you switch units). Each planner stage then gets a "carry up to X L" chip computed from its longest dry stretch at your pace, the trip brief prints a per-day carry line and a pack row on the cover, and the up-next water row shows the liters needed to reach the next source. An optional toggle slows all ETAs ~1% per kg of base weight above 8 kg. Off by default (leave base weight empty); fully offline
